            ...
            >
                {cars.map((car, index) => (
                    <div key={index} className="snap-center group shrink-0 w-full sm:w-87.5 h-100 relative rounded-2xl overflow-hidden bg-neutral-200 flex justify-center items-center">
                        ...
                        <div className="p-1 pr-2 rounded-md text-sm 2xl:text-base bg-neutral-800 text-white flex justify-center items-center gap-2"><User pack="filled" fill="#FFDB63" width={18} height={18}/><span>{car.passengers}</span></div>
                        <div className="p-1 pr-2 rounded-md text-sm 2xl:text-base bg-neutral-800 text-white flex justify-center items-center gap-2"><Luggage pack="filled" fill="#FFDB63" width={18} height={18}/><span>{car.luggage}</span></div>
                        </div>

                        <div className="w-full flex flex-col justify-center items-center absolute bottom-3 gap-1 z-30 ">
                            <h3 className="text-lg text-neutral-800 text-center tracking-tighter font-semibold">{car.name}</h3>
                            <p className="text-sm 2xl:text-base h-11 text-neutral-700 w-[90%] text-center font-medium tracking-tight">{car.description}</p>
                        </div>
                        <Image src={car.image} alt={car.name} width={320} height={220} loading="lazy"
